...
export function PermissionsPage({ platform, nativeRuntime, systemProbeState, permissions, inputAuthorizationStale, inputDriver, onRequestPermission, onOpenSettings, onRefresh, onOpenDriver }: PermissionsPageProps) {
  ...
  return <div className="permissions-page">
    <header className="permissions-page-head">
      <div><span className="section-kicker">SYSTEM ACCESS</span><h2>系统权限</h2><p>查看授权状态，管理 Axonkey 运行所需的系统访问权限。</p></div>
    ...
    </> : platform === 'windows' ? <section className="permissions-platform-note"><ShieldCheck size={28} /><h3>Windows 输入服务</h3><p>按键映射通过输入驱动运行，需要安装驱动并在系统提示时授予管理员权限。</p><p>驱动状态：{!nativeRuntime ? '未检测' : loading ? '检测中' : failed ? '检测失败' : inputDriver.status === 'installed' ? '已安装' : inputDriver.status === 'restartRequired' ? '需要重启' : '需要检查'}</p><button type="button" className="dialog-secondary" onClick={onOpenDriver}>打开驱动设置</button></section>
      : <section className="permissions-platform-note"><Info size={28} /><h3>当前系统暂不支持</h3><p>请在 macOS 或 Windows 桌面版中配置系统权限。</p></section>}
  </div>
}
