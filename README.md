...
边缘范围为当前显示器顶部或左右两侧的 8 个屏幕坐标单位（Windows 为像素，macOS 为点），支持多显示器，顶部角落优先使用上边缘规则。滚动的边缘规则优先；未配置或全部步骤停用时，沿用该滚动方向在“任意位置”的规则，任意位置也未配置则保持原始输入。鼠标左右键不继承全局规则，未配置的边缘和非边缘区域均保持原始点击。“禁用此触发方式”会拦截输入并不发送输出。关闭全局开关时保持原始输入，取消尚未执行的鼠标映射。
