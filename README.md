...
Windows 编辑器提供 13 个按键。返回键和独立音量 `+ / -` 需要在首页或映射页开启“返回键与音量键支持”，在 Windows 授权窗口中选择“是”，再依次按下并松开“返回 → 音量加 → 音量减”确认本次连接的遥控器。这三个键随后支持单击、双击和长按映射。管理员权限仅用于按键辅助进程；每次启动应用后手动授权，取消授权不会关闭其他按键。安装包已携带 Frida DLL，无需安装 Python 或额外驱动。详见 [Windows 输入](./docs/WINDOWS_INPUT.md)。
