...
Axonkey 是一款支持小米 RC003 蓝牙遥控器和鼠标边缘滚动的本地映射控制台。macOS 版通过 IOKit 读取目标设备（`VID 0x2717` / `PID 0x32B8`）的原始 HID 报告，并用 CoreGraphics 与 AppKit 发送映射后的输入；Windows 版通过 Interception 过滤目标设备输入，并将 RC003 语音转发到 VB-CABLE。Windows 还提供默认关闭的 Frida 增强通道，用于读取返回和音量键。
...
当前支持小米 RC003 蓝牙遥控器和系统鼠标的上边缘和左右边缘滚动。macOS 可以配置全部 13 个已识别实体按键：语音、电源、四向、确认、返回、音量 `+ / -`、主页、菜单和 TV 键。返回键默认保持 Delete（退格）行为，音量键默认保持 macOS 系统音量行为与连续按压节奏，也可以改成其他单击、双击或长按映射。
...
在“映射”左侧的“映射设备”下拉框选择“鼠标”，选择上边缘、左边缘或右边缘，再分别配置滚动行为。上边缘支持向上、向下、向左、向右滚动；左右边缘各自支持向上、向下滚动。可复用现有按键、组合键、媒体控制、文本和多步骤序列；无需配置单击、双击或长按。

触发区域为指针所在显示器顶部或左右两侧的 8 个屏幕坐标单位（Windows 为像素，macOS 为点），支持多显示器，顶部角落优先使用上边缘映射。左右方向需要水平滚轮或横向滚动手势。未配置、全部步骤停用、离开边缘或关闭全局开关时，鼠标保持原始滚动；“禁用此触发方式”会拦截对应方向并不发送输出。自定义输出的滚轮事件不会再次触发映射。
...
映射修改自动保存；导入／导出包含所有设备，兼容旧版 RC003 配置；“恢复默认”只重置当前设备。鼠标初始所有触发项均保留原始滚动，具体行为由用户自行配置。

## 默认映射

Windows 和 macOS 均可将遥控器按键设为四向鼠标滚轮或鼠标左／中／右键：在“按键映射”中选择遥控器按键 → “单击” → “导航编辑” → 选择滚轮方向或鼠标按键。滚轮每次发送一格事件，实际滚动距离遵循系统和目标应用设置；鼠标点击作用于当前指针位置。

如果该按键仅配置一个启用的单击滚轮行为，且没有启用的双击或长按行为，按下立即滚动一格，保持 400 ms 后约每 80 ms 滚动一格，松开停止；若配置了其他手势，则按对应手势执行一次。原有默认映射不会自动改动。macOS 需要已授予辅助功能权限。
