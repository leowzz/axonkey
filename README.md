...
Windows 可将方向上、方向下分别设为鼠标滚轮：在“按键映射”中选择遥控器按键 → “单击” → “导航编辑” → “滚轮向上 / 滚轮向下”。每次发送一格滚轮事件，实际滚动距离遵循 Windows 和目标应用设置。

如果该按键仅配置一个启用的单击滚轮行为，且没有启用的双击或长按行为，按下立即滚动一格，保持 400 ms 后约每 80 ms 滚动一格，松开停止；若配置了其他手势，则按对应手势执行一次。原有默认映射不会自动改动。此功能目前仅支持 Windows，向权限更高的应用发送滚轮事件可能被系统阻止。
