...
2. 进入“按键映射”，展开底部“高级选项”，阅读“返回与音量键增强”的兼容性说明后自行决定是否点击“开启并授权”。未开启时三个键标注“需开启增强”，选中后可点击“查看说明并开启”直达。默认加减音量同样需要增强支持，仅保存映射不会启用。开启后 Windows 会弹出 UAC，选择“是”。
...
三个额外按键的两段 TCP 通道都关闭小包合并（`TCP_NODELAY`），收到完整报告即转发。
