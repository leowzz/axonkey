...
3. 仅操作 RC003，依次按下并松开“返回 → 音量加 → 音量减”。确认期间不执行映射。
4. 显示“三个按键已启用”后，可编辑这三个键的行为。

应用主体保持普通权限，只有不带界面的采集辅助进程获得管理员权限。拒绝授权、
组件加载失败和连接超时都会显示原因，并提供重新授权入口。每次启动应用后需
手动授权；保存开关偏好或导入映射不会自动触发 UAC。关闭开关、关闭自定义按键
...
再由用户的三个完整按键动作选择本次会话的报告流。UMDF 代理本身不带可验证的
RC003 VID/PID，所以这是用户确认，不是自动的硬件身份认证。其他报告流不进入
映射；句柄关闭、设备对象或宿主变化后必须重新确认。不会持久化代理名或句柄。
