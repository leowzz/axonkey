...
const script = execFileSync(process.argv[2] || 'python', ['-B', '-c',
  'from rc003_hid.frida_hid_tap_runtime import GADGET_SCRIPT; print(GADGET_SCRIPT)'],
{ cwd: sourceDir, encoding: 'utf8' });
const sent = [];
const reads = [];
let callbacks, attached = 0, detached = 0, available = false;
...
const context = vm.createContext({
  ...
  Interceptor: { attach(_target, listeners) { callbacks = listeners; attached++; return { detach() { detached++; } }; } },
...
await context.rpc.exports.init(null, { port: 30685, protocol_id: 'test-revision' });
...
const config = new TextEncoder().encode(JSON.stringify({kind:'configure', devices:['\\device\\000000ee']}) + '\n');
reads.shift()(config.buffer);
const drain = async () => { for (let index = 0; index < 12; index++) await Promise.resolve(); };
await drain();
assert.equal(attached, 1);
const uint = n => ({ toUInt32() { return n; } });
const report = {
  isNull() { return false; },
...
await drain();
assert.equal(detached, 1, 'EOF detaches even when no further keys are pressed');
await vm.runInContext('connectToHub()', context);
assert.equal(attached, 1, 'reconnection requires a fresh allowlist');
reads.shift()(config.buffer);
await drain();
